...
import { IconChevronLeft, IconChevronRight, IconPlus, IconTrash, IconEdit } from '../components/Icon';
...
const PlannerPage: React.FC = () => {
  const { ingredients, recipes, getDailyPlan, updateDailyPlan, addItemToMeal, removeItemFromMeal, updateItemInMeal, getIngredientById, getRecipeById } = useData();
  ...
  const [targetNutrients, setTargetNutrients] = useState<NutrientInfo>({ 
    Energia: 2000, 
    Proteína: 100, 
    Carboidrato: 250, 
    Lipídeos: 70, 
  ...
  const handleAddItem = () => {
    if (selectedDayPlan && selectedMealType && currentItem.itemId && currentItem.type && currentItem.quantity) {
      const newItem: Omit<PlannedItem, 'id'> = {
        type: currentItem.type,
        itemId: currentItem.itemId,
        quantity: currentItem.quantity,
        customName: currentItem.customName
      };
      addItemToMeal(selectedDayPlan.date, selectedMealType, newItem);
  ...
  const handleEditItem = () => {
    if (selectedDayPlan && selectedMealType && currentItem.id && currentItem.itemId && currentItem.type && currentItem.quantity) {
        ...
        updateItemInMeal(selectedDayPlan.date, selectedMealType, updatedItem);
        setSelectedDayPlan(getDailyPlan(selectedDayPlan.date)); 
        closeModal();
    }
  };
  ...
  const filteredItems = React.useMemo(() => {
    ...
    ingredients
      .filter(ing => ing.name.toLowerCase().includes(lowerSearchTerm))
      .forEach(ing => searchResults.push({ id: ing.id, name: ing.name, type: 'ingredient', details: ing }));
      
    recipes
      .filter(rec => rec.name.toLowerCase().includes(lowerSearchTerm))
      .forEach(rec => searchResults.push({ id: rec.id, name: rec.name, type: 'recipe', details: rec }));
  ...
  return (
    <div className="space-y-6">
      ...
      <details className="bg-white p-4 rounded-lg shadow">
        <summary className="font-semibold text-emerald-600 cursor-pointer">Metas Nutricionais Diárias</summary>
          ...
          {nutrientLabels.map(({key, label, unit}) => (
            <div key={key}>
              ...
              <input
                ...
                id={`target-${key}`}
                name={key}
                value={targetNutrients[key]}
                onChange={(e) => setTargetNutrients(prev => ({ ...prev, [key]: parseFloat(e.target.value) || 0 }))}
      ...
      <div className="grid grid-cols-7 gap-1 sm:gap-2 bg-white p-2 rounded-lg shadow">
        {weekDates.map(date => (
          <button
            ...
            className={`p-2 sm:p-3 rounded-md text-center transition-colors duration-150
              ${formatDateISO(date) === formatDateISO(currentDate) ? 'bg-emerald-500 text-white font-semibold shadow-md' : 'bg-gray-100 hover:bg-emerald-100 text-gray-700'}
            `}
          >
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg">
        ...
        <div className="mb-6 p-4 bg-emerald-50 rounded-lg border border-emerald-200 space-y-2">
            <h3 className="text-sm font-semibold text-emerald-700 mb-1">Resumo Nutricional do Dia (vs Metas)</h3>
            {nutrientLabels.map(({key, label, unit, color}) => (
                 <NutrientBar key={key} label={label} value={dayTotalNutrients[key]} max={targetNutrients[key]} color={color} unit={unit} />
            ))}
        </div>
                ...
                <ul className="space-y-2">
                  {meal.items.map(item => {
                    ...
                    if (item.type === 'ingredient' && details) {
                        ...
                        const divisor = (ing.unit === '100g' || ing.unit === '100ml') ? 100 : 1;
                        itemNutrients = {
                    ...
                    } else if (item.type === 'recipe' && details) {
                        const rec = details as Recipe; 
                        if (rec.totalNutrients && rec.servings > 0) {
                           itemNutrients = {
                                Energia: (rec.totalNutrients.Energia / rec.servings) * item.quantity,
                                Proteína: (rec.totalNutrients.Proteína / rec.servings) * item.quantity,
                                Carboidrato: (rec.totalNutrients.Carboidrato / rec.servings) * item.quantity,
                                Lipídeos: (rec.totalNutrients.Lipídeos / rec.servings) * item.quantity,
                                Colesterol: (rec.totalNutrients.Colesterol / rec.servings) * item.quantity,
                                FibraAlimentar: (rec.totalNutrients.FibraAlimentar / rec.servings) * item.quantity,
                            };
                        }
                    }
                      ...
                      <li key={item.id} className="flex justify-between items-start p-3 bg-white rounded shadow-sm hover:shadow-md transition-shadow">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            E: {itemNutrients.Energia.toFixed(0)}Kcal, P: {itemNutrients.Proteína.toFixed(1)}g, C: {itemNutrients.Carboidrato.toFixed(1)}g, L: {itemNutrients.Lipídeos.toFixed(1)}g, Col: {itemNutrients.Colesterol.toFixed(0)}mg, FA: {itemNutrients.FibraAlimentar.toFixed(1)}g
                          </p>
                        </div>
                        <div className="flex space-x-2">
                          <Button onClick={() => openModal(meal.mealType, item)} variant="ghost" size="sm" aria-label="Editar item"><IconEdit /></Button>
                          <Button onClick={() => handleRemoveItem(meal.mealType, item.id)} variant="danger" size="sm" aria-label="Remover item"><IconTrash /></Button>
                        </div>
                      </li>
                    );
                  })}
              ...
              {meal.totalNutrients && (
                <div className="mt-3 pt-3 border-t border-gray-100">
                    <p className="text-xs text-gray-600 text-right">
                        Total Refeição: 
                        E: {meal.totalNutrients.Energia.toFixed(0)} Kcal, 
                        P: {meal.totalNutrients.Proteína.toFixed(1)}g, 
                        C: {meal.totalNutrients.Carboidrato.toFixed(1)}g, 
                        L: {meal.totalNutrients.Lipídeos.toFixed(1)}g,
                        Col: {meal.totalNutrients.Colesterol.toFixed(0)}mg,
                        FA: {meal.totalNutrients.FibraAlimentar.toFixed(1)}g
                    </p>
                </div>
              )}
            </div>
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title={modalMode === 'add' ? 'Adicionar Item à Refeição' : 'Editar Item da Refeição'} size="lg">
        <div className="space-y-4">
          <div>
            <label htmlFor="itemSearch" className="block text-sm font-medium text-gray-700">Buscar Ingrediente ou Receita</label>
            <input
              type="text"
              id="itemSearch"
              value={searchTerm}
            ...
            <div className="max-h-48 overflow-y-auto border border-gray-200 rounded-md">
              {filteredItems.map(resItem => (
                <button
                  key={resItem.id}
                  onClick={() => {
                    setCurrentItem(prev => ({ ...prev, itemId: resItem.id, type: resItem.type, customName: resItem.name }));
                    setSearchTerm(''); 
                  }}
                  className={`block w-full text-left px-3 py-2 text-sm hover:bg-emerald-50 transition-colors
                    ${currentItem.itemId === resItem.id ? 'bg-emerald-100 text-emerald-700 font-semibold' : 'text-gray-700'}`}
                >
                  {resItem.name} <span className="text-xs text-gray-500">({resItem.type === 'ingredient' ? 'Ingrediente' : 'Receita'})</span>
                </button>
              ))}
            </div>
          )}
           {currentItem.itemId && (
            <p className="text-sm text-emerald-700">Selecionado: {currentItem.customName || (currentItem.type === 'ingredient' ? getIngredientById(currentItem.itemId || '')?.name : getRecipeById(currentItem.itemId || '')?.name)}</p>
          )}
          <div>
            <label htmlFor="quantity" className="block text-sm font-medium text-gray-700">
              Quantidade ({currentItem.type === 'ingredient' ? (getIngredientById(currentItem.itemId || '') as Ingredient)?.unit || 'unidade' : 'porções'})
            </label>
            <input
              type="number"
              id="quantity"
          ...
          <div>
            <label htmlFor="customName" className="block text-sm font-medium text-gray-700">Nome Personalizado (Opcional)</label>
            <input
              type="text"
              id="customName"
              value={currentItem.customName || ''}
              onChange={(e) => setCurrentItem(prev => ({ ...prev, customName: e.target.value }))}
              placeholder="Ex: Maçã grande, Salada especial"
              className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-emerald-500 focus:border-emerald-500"
            />
          </div>
        </div>
        <div className="mt-6 flex justify-end space-x-3">
          ...
          <Button onClick={modalMode === 'add' ? handleAddItem : handleEditItem} disabled={!currentItem.itemId || !currentItem.quantity || currentItem.quantity <= 0}>
